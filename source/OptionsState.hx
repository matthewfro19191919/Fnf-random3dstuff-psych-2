...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				switch(options[curSelected]) {
					...
					case '3D Quality':
						ClientPrefs.quality3D += add * 0.125;
						if(ClientPrefs.quality3D > 3) ClientPrefs.quality3D = 3; else
						if(ClientPrefs.quality3D < 1) ClientPrefs.quality3D = 1;
					case 'Note Delay':
						var mult:Int = 1;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case '3D Quality':
				daText = "Higher the number, the worst the quality (division).\nTHIS WILL REQUIRE A RESTART OF THE GAME.\nChange this to increase 3D performance.";
			case 'Note Delay':
